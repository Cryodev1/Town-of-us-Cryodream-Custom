...
using TownOfUs.NeutralRoles.GAMod;
...
namespace TownOfUs
{
    public static class RpcHandling
    {
        ...
        private static void GenGA(List<GameData.PlayerInfo> infected, List<PlayerControl> crewmates)
        {
            PlayerControl pc;
            var targets = Utils.getCrewmates(infected).Where(x =>
            {
            ...
            if (targets.Count > 1)
            {
                var rand = Random.RandomRangeInt(0, targets.Count);
                pc = targets[rand];
                var role = Role.Gen(typeof(GA), crewmates.Where(x => x.PlayerId != pc.PlayerId).ToList(),
                    CustomRPC.SetGA);
                if (role != null)
                {
                    crewmates.Remove(role.Player);
        ...
        private static void GenEachRole(List<GameData.PlayerInfo> infected)
        {
            ...
            if (GAOn) GenGA(infected, crewmates);
            ...
            public static void Postfix([HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
            {
                ...
                switch ((CustomRPC) callId)
                {
                    ...
                    case CustomRPC.ExecutionerToJester:
                        ...
                        break;
                    case CustomRPC.GAToJester:
                        GATargetColor.GAToJes(Utils.PlayerById(reader.ReadByte()));
                        break;
        ...
        public static class RpcSetInfected
        {
            public static void Prefix([HarmonyArgument(0)] ref Il2CppReferenceArray<GameData.PlayerInfo> infected)
            {
                ...
                if (Check(CustomGameOptions.GAOn))
                    NeutralRoles.Add((typeof(GA), CustomRPC.SetGA, CustomGameOptions.GAOn));
