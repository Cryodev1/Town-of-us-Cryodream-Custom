...
namespace TownOfUs.NeutralRoles.GAMod
{
    ...
    {
        private static void UpdateMeeting(MeetingHud __instance, GA role)
        {
        ...
        private static void Postfix(HudManager __instance)
        {
            ...
            if (!PlayerControl.LocalPlayer.Is(RoleEnum.GA)) return;
            var role = Role.GetRole<GA>(PlayerControl.LocalPlayer);

            if (role.target == null)
            {
                var writer2 = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId,
                    (byte) CustomRPC.GAToJester, SendOption.Reliable, -1);
                writer2.Write(PlayerControl.LocalPlayer.PlayerId);
                AmongUsClient.Instance.FinishRpcImmediately(writer2);
                GAToJes(PlayerControl.LocalPlayer);
                return;
            }
        ...
        public static void GAToJes(PlayerControl player)
        {
            player.myTasks.RemoveAt(0);
            Role.RoleDictionary.Remove(player.PlayerId);


            if (CustomGameOptions.OnGATargetDead == OnGATargetDead.Jester)
            {
